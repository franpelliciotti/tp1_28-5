...
public class CodificadorMensajes
{
    ...
    private int[] generarCodigoEncripcion(String str) 
    {
        ...
        int contador = 0;
        while(i < str.length()){
            contador += str.charAt(i); //suma y acumula los valores ASCII de cada letra
            i++;
        }
        contador = contador % 99991;
        String contModStr = String.valueOf(contador); //paso a str por int cannot be dereferenced
        int[] acumulador = new int[contModStr.length()]; //creación array que recibirá cada caracter
        
        for(int j = acumulador.length -1; j >= 0; j--){ //Extraer cada número según su posición como potencia decimal.
            acumulador[j] = contador % 10;
            contador = contador / 10;
        }
        
        codigoEncripcion = acumulador;
        return acumulador;
    }
